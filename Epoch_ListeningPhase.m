...
current_interested_event = 'Whole_listening_period';
...
%% Epoch Whole_listening_period: 

if strcmp(current_interested_event,'Whole_listening_period')==1 
    ...
    for subID_idx = 1:totalSub
        ...
        %------------------------- Epoching Whole_listening_period -----------------------------%        
            % we want to epoch [0, 1000 ms] for each trial; 
            % i.e., epoch 512 timepoints post S1 (including S2)
            % which means there will be 512 elements in each cell
            for trial_idx = 1:total_trials
                ...
                if current_trial_latency == 0 || isnan(current_trial_latency)  % missing trial
                    ...
                    extracted_epoch_cell_array{subID_idx, trial_idx} = EEG.data(:, current_trial_latency :(current_trial_latency + 1*sampling_rate - 1)); % we are NOT using pop_epoch here because we have already got all the time points information
                    extracted_time_cell_array{subID_idx, trial_idx} = EEG.times(:, current_trial_latency :(current_trial_latency + 1*sampling_rate - 1));
                end
            end
...
save('Whole_listening_period_epoch_cell_array.mat', ... % this is the name of the workspace
...
%% each trial length
each_trial_interval_length = nan(totalSub, 100);

for subID_idx = 1:totalSub
    current_subID = subID_list(subID_idx);
    if ismember(current_subID, excluded_subj_list)==0  
        for trial_idx = 1:99  
            if all_latency_S1(subID_idx, trial_idx+1)~=0 && all_latency_S1(subID_idx, trial_idx)~= 0
                each_trial_interval_length(subID_idx, trial_idx) = (all_latency_S1(subID_idx, trial_idx+1) - all_latency_S1(subID_idx, trial_idx) + 1)/sampling_rate;
            end
        end
    end
end
...
figure
hold on
histogram(each_trial_interval_length, 30)

max(max(each_trial_interval_length))
max(each_trial_interval_length(:, 59))
find(each_trial_interval_length > 10)
find(max(each_trial_interval_length) > 10)

for subID_idx = 1:totalSub
    current_subID = subID_list(subID_idx);
    disp(['current_subID = ', num2str(current_subID)])
    max(each_trial_interval_length(subID_idx, :))
end
